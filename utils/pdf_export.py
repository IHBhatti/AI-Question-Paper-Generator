...
def _header_block(meta: Dict) -> list:
    ...
    if meta.get("institution_name"):
        elements.append(Paragraph(meta["institution_name"], TITLE_STYLE))
    elements.append(Paragraph(meta.get("subject", "Examination Paper"), SUBTITLE_STYLE))
    elements.append(Spacer(1, 6))
    elements.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor("#1a1a2e")))
    elements.append(Spacer(1, 8))

    info_rows = [
        ["Subject:", meta.get("subject", "-"), "Total Marks:", str(meta.get("total_marks", "-"))],
        ["Exam Date:", meta.get("exam_date", "-"), "Time Duration:", meta.get("time_duration", "-")],
        ["Chapters Covered:", ", ".join(meta.get("chapters", [])) or "Entire Document", "", ""],
    ]
    ...
    if meta.get("instructions"):
        ...
        for line in meta["instructions"].split("\n"):
            if line.strip():
                elements.append(Paragraph(f"• {line.strip()}", INSTRUCTIONS_STYLE))
        elements.append(Spacer(1, 8))
...
def _mcq_section(mcqs: List[dict], marks_per_question: int) -> list:
    ...
    for i, q in enumerate(mcqs, start=1):
        ...
        for letter, option_text in q.get("options", {}).items():
            elements.append(Paragraph(f"({letter}) {option_text}", OPTION_STYLE))
...
def export_answer_key_pdf(
    ...
    story = [
        Paragraph(f"Answer Key — {meta.get('subject', '')}", TITLE_STYLE),
        Spacer(1, 10),
    ...
    if mcqs:
        ...
        for q in mcqs:
            story.append(Paragraph(f"<b>{counter}. {q.get('question', '')}</b>", QUESTION_STYLE))
            story.append(
                Paragraph(
                    f"Correct Option: <b>{q.get('correct_option', '-')}</b> — {q.get('explanation', '-')}",
                    OPTION_STYLE,
                )
            )
            story.append(Spacer(1, 6))
    ...
    if short_questions:
        ...
        for q in short_questions:
            story.append(Paragraph(f"<b>{counter}. {q.get('question', '')}</b>", QUESTION_STYLE))
            story.append(Paragraph(q.get("model_answer", "-") or "-", OPTION_STYLE))
            story.append(Spacer(1, 6))
            counter += 1
        counter = 1
    ...
    if long_questions:
        ...
        for q in long_questions:
            ...
            if points:
                ...
                story.append(Paragraph(f"<i>Key marking points: {'; '.join(points)}</i>", OPTION_STYLE))
