...
def load_chunks_cache(name: str) -> Optional[List[Document]]:
    ...
    try:
        ...
        logger.warning("Cached chunks at %s failed to load (%s); falling back to reprocessing", path, e)
        return None


def load_vectorstore(name: str) -> Optional[FAISS]:
    """Load a previously saved FAISS index, if a complete one exists.

    Returns None (rather than raising) for anything short of a fully
    intact cache — a missing directory, a partial write (e.g. chunks
    were cached but the vectorstore save was interrupted), or a load
    error from a corrupted file. A cache miss should always fall back
    to normal reprocessing, never crash the whole "Process Document"
    flow.
    """
    out_dir = VECTORSTORE_DIR / name
    if not (out_dir / "index.faiss").exists() or not (out_dir / "index.pkl").exists():
        return None
    try:
        embeddings = get_embedding_model()
        return FAISS.load_local(str(out_dir), embeddings, allow_dangerous_deserialization=True)
    except Exception as e:
        logger.warning("Cached vector store at %s exists but failed to load (%s); "
                        "falling back to reprocessing", out_dir, e)
        return None
...
def is_duplicate_document(file_hash: str) -> bool:
    ...
    try:
        ...
        logger.warning("Duplicate-check index at %s failed to load (%s); assuming not a duplicate", index_path, e)
        return False
...
def register_document_hash(file_hash: str) -> None:
    ...
    if index_path.exists():
        try:
            ...
            logger.warning("Existing hash index at %s was unreadable (%s); starting a fresh one", index_path, e)
            hashes = set()
